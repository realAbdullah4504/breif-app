  ...
  try {
    ...
    for (const workspace of workspaces){
      ...
      for (const member of relevantMembers){
        ...
        const { data: todayBriefs, error: briefsError } = await supabaseAdmin.from("briefs").select("submitted_at").eq("user_id", member.id).gte("submitted_at", `${today}T00:00:00`);
        if (briefsError) {
        ...
        if (todayBriefs.length === 0) {
          ...
          await supabaseAdmin.functions.invoke("send-email", {
            body: {
              ...
              html: workspace.reminder_template.body.replace("{{name}}", member.name || member.email.split("@")[0]).replace("{{deadline}}", workspace.submission_deadline)
            }
          });
        } else {
