...
export class BriefService {
  async submitBrief(
    ...
    user_id: string,
    workspaceId: string
  ): Promise<{ data: Brief | null; error: Error | null }> {
    try {
  ...
  async sendBriefNotificationToAdmin(
    brief: Brief,
    user: any,
    workspaceId: string
  ): Promise<void> {
    try {
      ...
      const { data: memberData, error: memberError } = await supabase
        ...
        .select("user_id,users:invited_by (email, name)")
        .eq("user_id", user.id)
          ...
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
              ...
              <div style="background: #f8fafc; border-radius: 12px; padding: 20px; margin-bottom: 24px; border: 1px solid #e2e8f0;">
                <div style="display: flex; align-items: center;">
                  <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #6366f1, #8b5cf6); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 16px;">
                    <span style="color: white; font-size: 20px; font-weight: 600;">${user.name
                      .charAt(0)
                      .toUpperCase()}</span>
                  </div>
                  <div>
                    ...
                    <p style="color: #64748b; margin: 4px 0 0 0; font-size: 14px;">Submitted ${format(
                      new Date(brief.submitted_at),
                      "EEEE, MMMM d"
                    )} at ${format(new Date(brief.submitted_at), "h:mm a")}</p>
                  </div>
                </div>
              </div>
              ...
              <div style="margin-bottom: 32px;">
                ...
                <div style="margin-bottom: 20px;">
                  ...
                  <div style="background: #f0fdf4; padding: 16px; border-radius: 0 0 8px 8px; border: 1px solid #bbf7d0; border-top: none;">
                    <p style="color: #166534; margin: 0; line-height: 1.5; font-size: 14px;">${
                ...
                <div style="margin-bottom: 20px;">
                  ...
                  <div style="background: #fef2f2; padding: 16px; border-radius: 0 0 8px 8px; border: 1px solid #fecaca; border-top: none;">
                    <p style="color: #991b1b; margin: 0; line-height: 1.5; font-size: 14px;">${
                      brief.blockers || "No blockers reported"
                    }</p>
                  </div>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                  ...
                  <div style="background: #eff6ff; padding: 16px; border-radius: 0 0 8px 8px; border: 1px solid #bfdbfe; border-top: none;">
                    <p style="color: #1e40af; margin: 0; line-height: 1.5; font-size: 14px;">${
                      brief.priorities
                    }</p>
                  </div>
                </div>
            ...
            <div style="background: #f8fafc; padding: 20px 24px; text-align: center; border-top: 1px solid #e2e8f0;">
              <p style="color: #64748b; margin: 0; font-size: 12px; line-height: 1.4;">
                This email was sent when <strong>${
                  user.name
                }</strong> submitted their daily brief.<br>
  ...
  async getUserBriefs(
    userId: string,
    workspaceId: string
  ): Promise<{ data: Brief[]; error: Error | null }> {
    try {
      const { data, error } = await supabase
        .from("briefs")
        .select("*")
