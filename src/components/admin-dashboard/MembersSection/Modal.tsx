...
import { formatWorkspaceDate, formatWorkspaceTime } from '../../../utils/workspaceTimeUtils'
...
const MemberModal = ({ isDarkMode, settings, selectedBrief, isModalOpen, setIsModalOpen }: ModalProps) => {
    const { reviewBrief, isReviewing } = useReviewBriefs();
    ...
    const handleMarkAsReviewed = (briefId: string, userId: string) => {
        ...
        if (briefId.startsWith('demo-') || userId.startsWith('demo-')) {
            toast.error("This is demo data. Invite real team members to review their briefs!");
            return;
        }
    ...
    const handleDownloadBrief = () => {
        ...
        try {
            ...
            // Add submission info
            doc.setFontSize(12);
            doc.text(`Submitted on: ${format(new Date(selectedBrief.submitted_at), 'MMMM d, yyyy h:mm a')}`, 20, yPos);
            yPos += lineHeight * 2;
            ...
            addSection(settings.questions.accomplishments, selectedBrief.accomplishments);
            addSection(settings.questions.blockers, selectedBrief.blockers || 'None reported');
            ...
            if (settings.questions.question4 && selectedBrief.question4_response) {
                addSection(settings.questions.question4, selectedBrief.question4_response);
            }

            if (settings.questions.question5 && selectedBrief.question5_response) {
                addSection(settings.questions.question5, selectedBrief.question5_response);
            }

            // Save the PDF
            doc.save(`brief-${selectedBrief.users?.name}-${format(new Date(selectedBrief.submitted_at), 'yyyy-MM-dd')}.pdf`);
        } catch (error) {
                ...
                <div className="space-y-4">
                        ...
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    {formatWorkspaceDate(selectedBrief.submitted_at, settings.timezone)} • Submitted at {formatWorkspaceTime(selectedBrief.submitted_at, settings.timezone)}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="pt-4 flex justify-between">
                        ...
                        <div className="flex space-x-2">
                            {!selectedBrief?.reviewed_by && (
                                <Button
                                    onClick={() => handleMarkAsReviewed(selectedBrief.id, selectedBrief.user_id)}
